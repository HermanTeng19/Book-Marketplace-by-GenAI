...
const Login = () => {
  ...
  // Get redirect path from location state or determine by user role
  const getRedirectPath = (user) => {
    if (!user) return '/dashboard';
    
    // If there's a specific path in location state, use that first
  ...
  // Redirect if already logged in, but only once auth is no longer loading
  useEffect(() => {
  ...
  }, [user, authLoading, isAuthenticated, navigate, redirected]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        // Login success - the redirect will happen via the user effect
